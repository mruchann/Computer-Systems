...
long long gen_need_regids()
{
    return ((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
      (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == (I_IRMOVQ) || (icode)
       == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == (I_CMPQ));
}
...
long long gen_srcA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_ALU) || (icode) == (I_PUSHQ) || (icode) == (I_CMPQ)) ? (ra) : (
...
long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_MRMOVQ) || (icode) == (I_CMPQ)) ? (rb) : ((icode) == (I_PUSHQ)
...
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
        (I_CMPQ)) ? (vala) : ((icode) == (I_IRMOVQ) || (icode) == 
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == 
        (I_ALU) || (icode) == (I_CALL) || (icode) == (I_PUSHQ) || (icode)
         == (I_RET) || (icode) == (I_POPQ) || (icode) == (I_CMPQ)) ? (valb)
       : ((icode) == (I_RRMOVQ) || (icode) == (I_IRMOVQ)) ? 0 : 0);
}

long long gen_alufun()
{
    return (((icode) == (I_ALU)) ? (ifun) : ((icode) == (I_CMPQ)) ? (A_SUB)
       : (A_ADD));
}

long long gen_set_cc()
{
    return ((icode) == (I_ALU) || (icode) == (I_CMPQ));
}
