...
instr_t instruction_set[] = 
{
    ...
    {"cmpq",   HPACK(I_CMPQ, F_NONE), 2, R_ARG, 1, 1, R_ARG, 1, 0},
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	s->pc = ftpc;
	break;
    /* Let's try to simulate cmpq here! Sure lots of copy-paste... (DS) */
	...
	if (!okc) {
	    if (error_file)
		fprintf(error_file,
			"PC = 0x%llx, Invalid instruction address", s->pc);
	    return STAT_INS;
	}
	if (!reg_valid(hi1)) {
	    if (error_file)
